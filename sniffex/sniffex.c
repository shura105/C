...
int decode_request(const char *payload, int len)
{
	...
	char buffer[strlen(payload)];
        ...
        char* header[] = {"User-Agent: ", "Host: ", "Referer: ", "X-Forwarded-For: ", "Cookie: ", "Accept: "};
        char *start = buffer, *next=NULL;
        while(NULL != start && (NULL != (next=strstr(start, "\r\n"))))
        {
                *next = '\0';
			...
			http = 1;
                ...
                for (i = 0; i < 6; i++)
                {
                        if(!strncasecmp(start, header[i], strlen(header[i])))
                        {
                                printf("%11s%s\n", header[i],start + strlen(header[i]));
                                next += 2;
                                start = next;
                                break;
                        }
 ...
 * 	 	 */
	...
	printf("   flag    : %x ", tcp->th_flags);
 	char* flags[] = {", FIN",",SYN",",RST",",PSH",",ACK",",URG"};
        int i;
        for (i = 0; i < 6; i ++) {
            if (tcp->th_flags >> i & 1) {
                printf("%s", flags[i]);
	...
	printf("Acknowledgment Number: %llu\n", ntohl(tcp->th_ack));	
...
int main(int argc, char **argv)
{
	...
	print_app_banner();
	if (argc != 3) {
	    printf("Usage: ./sniffer eth1 'host 12.1.1.1'");
	    return;
        }
